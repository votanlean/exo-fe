...
contract('TEXOOrchestrator', ([owner, dev, fee, staker1, referrer]) => {
  beforeEach(async () => {
    ...
    this.tEXOOrchestrator = await TEXOOrchestrator.new(
      ...
      dev,
      fee,
      latestBlock.toNumber() + 24 * 6, // Start allow reducing emission rate after 6 days,
      {
        from: owner,
      },
    );
    ...
    this.tEXOOrchestrator.add(
      ...
      '400', // Deposit fee
      false, // With update
      latestBlock.toNumber() + 24 * 5, // Block to receive rewards: 5 days
      latestBlock.toNumber(), // Start generate rewards block, immediately;
    ...
    this.tEXOOrchestrator.add(
      ...
      '0', // Deposit fee
      false, // With update
      latestBlock.toNumber(), // Can receive rewards immediately
  ...
  describe('Initialization phase tests', () => {
    ...
    it('can update global blockToReduceEmissionRate', async () => {
      const latestBlock = await time.latestBlock();
      newBlockToStartReducingEmissionRate = latestBlock.toNumber() + 10000;
      await this.tEXOOrchestrator.setBlockToStartReducingEmissionRate(
        newBlockToStartReducingEmissionRate,
        {
          from: owner,
        },
      );
      ...
      const updatedBlock =
        await this.tEXOOrchestrator.blockToStartReducingEmissionRate();
      expect(updatedBlock.toNumber()).to.be.equal(
        newBlockToStartReducingEmissionRate,
        'Cannot update blockToStartReducingEmissionRate',
      );
    });
  });
  ...
  describe('Staking phase tests', () => {
    ...
    it('Cannot stake exceed approval amount', async () => {
      let hasError = false;
      try {
        await this.bUSDInstance.approve(this.tEXOOrchestrator.address, '600', {
          from: staker1,
        });
    ...
    it('Allow stake if amount is good', async () => {
      let hasError = false;
      try {
        await this.bUSDInstance.approve(this.tEXOOrchestrator.address, '800', {
          from: staker1,
        });
  ...
  describe('Claim rewards & withdraw phase', () => {
    describe('Seeding pool tests', () => {
      ...
      it("Can't yield tEXO before startBlock", async () => {
        const staker1PendingTEXO = await this.tEXOOrchestrator.pendingTEXO(
          '0',
          staker1,
        );
        ...
        expect(staker1PendingTEXO.eq(new BN(0))).to.be.equal(
          true,
          'Should not yield tEXO before startblock',
        );
      });
      ...
      it('Yield tEXO after startBlock', async () => {
        ...
        await time.advanceBlockTo(
          currentBlock.toNumber() - this.offsetBlock + 9,
        );
      ...
      it('Should reduce BUSD balance after staking', async () => {
        const userBUSDBalanceAfterStake = await this.bUSDInstance.balanceOf(
          staker1,
        );

        expect(userBUSDBalanceAfterStake.toNumber()).to.be.equal(
          200,
          'User should have spent 800 BUSD',
        );
      });
      ...
      it('Withdraw should have zero tEXO rewards when performed after startBlock but before claim rewards block', async () => {
        ...
        await time.advanceBlockTo(
          latestBlock.toNumber() - this.offsetBlock + 9,
        );

        const beforeWithdrawRewards = await this.tEXOOrchestrator.pendingTEXO(
          '0',
          staker1,
        );
        ...
        const afterWithdrawRewards = await this.tEXOOrchestrator.pendingTEXO(
          '0',
          staker1,
        );

        expect(afterWithdrawRewards.gt(beforeWithdrawRewards)).to.be.equal(
          true,
        ...
        expect(userTEXORewards.eq(new BN(0))).to.be.equal(
          true,
          'User should not have tEXO rewards before withdraw time',
        );
      });
      ...
      it('Withdraw should yield tEXO rewards after claim rewards block', async () => {
        ...
        expect(userTEXORewards.gt(new BN(0))).to.be.equal(
          true,
          'User should have tEXO rewards after claim rewards time',
        );
      });
      ...
      it('Cannot stake after staking phase', async () => {
        ...
        await expectRevert(
          this.tEXOOrchestrator.deposit('0', '0', constants.ZERO_ADDRESS, {
            from: staker1,
          }),
          'deposit: staking period has ended',
        );
      });
      ...
      it("Should preserve user's reward after staking phase", async () => {
        ...
        const userPendingRewardLastDay =
          await this.tEXOOrchestrator.pendingTEXO('0', staker1);

        expect(
          userPendingRewardLastDay.gt(new BN(0)),
          'User should have pending reward',
        );

        await time.advanceBlock();

        const userPendingRewardExceed1Day =
          await this.tEXOOrchestrator.pendingTEXO('0', staker1);

        expect(
          userPendingRewardLastDay.eq(userPendingRewardExceed1Day),
        ).to.be.equal(true, 'Should not increase reward after 30 days');

        await this.tEXOOrchestrator.withdraw('0', '600', { from: staker1 });

        const userRewardBalance = await this.tEXOInstance.balanceOf(staker1);

        expect(userRewardBalance.eq(userPendingRewardLastDay)).to.be.equal(
          true,
          'User should have FAANG balance equal to pending reward',
        );
      });
    });
    ...
    describe('LP farm tests', () => {
      ...
      it('Yield tEXO after staking phase ended', async () => {
        ...
        const staker1PendingTEXORightAtStakingPhase =
          await this.tEXOOrchestrator.pendingTEXO('1', staker1);
      ...
      it('Should reduce tEXO/BNB balance after staking', async () => {
        const userBUSDBalanceAfterStake =
          await this.tEXOBNBFarmInstance.balanceOf(staker1);

        expect(userBUSDBalanceAfterStake.eq(new BN(200))).to.be.equal(
          true,
          'User should have spent 800 BUSD',
        );
      });
  ...
  describe('Referral feature', () => {
    ...
    it('Should pay referrer correctly', async () => {
      ...
      await this.tEXOOrchestrator.withdraw('0', `${400 * 0.96}`, {
        from: staker1,
      });
      ...
      expect(referrerBalance.eq(staker1Balance.div(new BN(50)))).to.be.equal(
        true,
