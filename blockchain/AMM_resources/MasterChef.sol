...
// Plum Token with Governance.
contract PlumToken is BEP20('Plum Token', 'PLUM') {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "PLUM::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "PLUM::delegateBySig: invalid nonce");
        require(now <= expiry, "PLUM::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "PLUM::getPriorVotes: not yet determined");
        ...
        internal
    {
        ...
        uint256 delegatorBalance = balanceOf(delegator); // balance of underlying PLUMs (not scaled);
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "PLUM::_writeCheckpoint: block number exceeds 32 bits");
...
// MasterChef is the master of PLUM Token (PLUM). He can make PLUM and he is a fair guy.
//
...
// contract once $PLUM is sufficiently distributed and the community can show to govern itself.
//
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of PLUMs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accPlumPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accPlumPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. PLUMs to distribute per block.
        uint256 lastRewardBlock; // Last block number that PLUMs distribution occurs.
        uint256 accPlumPerShare; // Accumulated PLUMs per share, times 1e12. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
    }

    // The PLUM Token!
    PlumToken public plum;
    // Dev address.
    address public devAddr;
    // PLUM tokens created per block.
    uint256 public plumPerBlock;
    // Deposit Fee address
    ...
    // The block number when PLUM mining starts.
    ...
    // Initial emission rate: 1 PLUM per block.
    uint256 public constant INITIAL_EMISSION_RATE = 1 ether;
    // Minimum emission rate: 0.05 PLUM per block.
    ...
    constructor(
        ...
        plumPerBlock = INITIAL_EMISSION_RATE;
    ...
    // Update the given pool's PLUM allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    // View function to see pending PLUMs on frontend.
    function pendingPlum(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accPlumPerShare = pool.accPlumPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 multiplier =
                getMultiplier(pool.lastRewardBlock, block.number);
            uint256 plumReward =
                multiplier.mul(plumPerBlock).mul(pool.allocPoint).div(
                    totalAllocPoint
                );
            accPlumPerShare = accPlumPerShare.add(
                plumReward.mul(1e12).div(lpSupply)
            );
        }
        return user.amount.mul(accPlumPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        plum.mint(devAddr, plumReward.div(10));
        plum.mint(address(this), plumReward);
        pool.accPlumPerShare = pool.accPlumPerShare.add(
            plumReward.mul(1e12).div(lpSupply)
        );
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for PLUM allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending =
                user.amount.mul(pool.accPlumPerShare).div(1e12).sub(
                    user.rewardDebt
                );
            if (pending > 0) {
                safePlumTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accPlumPerShare).div(1e12);
        emit Deposit(msg.sender, _pid, _amount);
    }

    // Deposit LP tokens to MasterChef for PLUM allocation with referral.
    ...
    // Safe Plum transfer function, just in case if rounding error causes pool to not have enough PLUMs.
    function safePlumTransfer(address _to, uint256 _amount) internal {
        uint256 plumBal = plum.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > plumBal) {
            transferSuccess = plum.transfer(_to, plumBal);
        } else {
            transferSuccess = plum.transfer(_to, _amount);
        }
    ...
    // Reduce emission rate by 3% every 14,400 blocks ~ 12hours till the emission rate is 0.05 PLUM. This function can be called publicly.
    function updateEmissionRate() public {
        ...
        require(plumPerBlock > MINIMUM_EMISSION_RATE, "updateEmissionRate: Emission rate has reached the minimum threshold");
        ...
        uint256 newEmissionRate = plumPerBlock;
        ...
        if (newEmissionRate >= plumPerBlock) {
            return;
        }

        massUpdatePools();
        lastReductionPeriodIndex = currentIndex;
        uint256 previousEmissionRate = plumPerBlock;
        plumPerBlock = newEmissionRate;
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        ...
        if (referrer != address(0) && referrer != _user && refBonusBP > 0) {
            ...
            plum.mint(referrer, refBonusEarned);
